/**
 * Chat client for the LucIA widget. Speaks the soul SSE wire contract over a
 * plain POST (`EventSource` cannot POST, so the stream is parsed by hand):
 * one TurnEvent per `data: <JSON>\n\n` message, `: ping` comments as
 * keep-alive, and the stream closes after `turn_done`.
 */
